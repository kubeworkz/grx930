...
#   mode: "quad" (default), "suite", "pta" or "mulstore" -- to tb_soc4 main().
#   dir:  the directory verilate-soc4 generated into (default build/verilator4;
#         PTM_C=1 puts it in build/ptm_c/verilator4).
#   env:  NAME=value pairs for the model, as one argument.  These cannot be
#         inherited: the model runs under wsl.exe, which does not pass the
#         calling shell's environment through, so they are put on its command
#         line here.
set -e
